...
@Component({
  selector: 'app-auto-complete-type-tiers',
  templateUrl: './auto-complete-type-tiers.component.html',
  styleUrls: ['./auto-complete-type-tiers.component.scss']
})
export class AutoCompleteTypeTiersComponent implements OnInit {
  @Input() items = {
    //num: "Code",
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if(this.bordureRed){
      if(document.getElementById(this.idMyInput) != null){
        document.getElementById(this.idMyInput).classList.add("border-erreur")
      }
    }else{
      if(document.getElementById(this.idMyInput) != null){
        document.getElementById(this.idMyInput).classList.remove("border-erreur")
      }
    }

    if(this.idSelected === ""){
      this.autocompleteValue = ""
      this.autocompleteValue2 = ""
    }
  ...
  checkLastElment(key, items){
    ...
    if(pos === this.objectKeys(items).length-1)
    return false
